...
/** How fast the axe/pickaxe/hammer arc cycles, in swings per second. Fishing is handled on its own
 *  clock below (`fishingToolSwing`) — a held rod does not swing on a beat the way a chopping or
 *  hammering tool does. Tuned by eye purely for how the animation reads; it has no bearing on
 *  `WORK_SECONDS` or any other simulation timing (Phase 10's "don't duplicate simulation logic in
 *  the renderer" cuts both ways — this must never be mistaken for an actual production rate). */
const SWING_HZ: Record<'woodcutting' | 'mining' | 'building', number> = {
  woodcutting: 0.62,
  mining: 0.7,
  building: 1.35, // a hammer taps quicker than an axe or pickaxe swings
};

/** A wide, plain-sine arc from raised-back to struck-down-forward and back. Shared by the axe and
 *  the pickaxe — at the few dozen pixels a villager stands on screen, what reads is the swinging
 *  motion and the silhouette of *something* in their hands, not a distinct arc shape per tool (the
 *  same reasoning the model already applies to leg/torso geometry, see `render/villager.ts`'s file
 *  doc comment). Returns the tool's absolute pitch in radians. */
function chopSwing(phase: number): number {
  return -0.65 + Math.sin(phase) * 0.95;
}

/** A hammer's arc is shorter as well as faster — `SWING_HZ.building` already carries the extra
 *  speed, so the reduced amplitude here is what also makes it read as a smaller tool. */
...
/** Jobs with an authored swing on the shared tool prop. Everything else in `VillagerActivity`
 *  (currently nothing — `fishing` gets its own clock above) falls through `showTool: false`. */
function isSwingJob(activity: VillagerActivity): activity is 'woodcutting' | 'mining' | 'building' {
  return activity === 'woodcutting' || activity === 'mining' || activity === 'building';
}
...
export function computeVillagerPose(c: Citizen, moving: boolean, now: number): VillagerPose {
  ...
  if (activity && isSwingJob(activity)) {
    ...
    return { state: 'working', toolSwing: activity === 'building' ? hammerSwing(phase) : chopSwing(phase), showTool: true };
  }
