...
| B3 | **`CONSUMPTION_SLOWDOWN` = 3** cut demand by two-thirds with no supply-side change — the village may now be *too* easy to feed/heat. | 🔵 needs-playtest | Newest and largest economy dial. Phase-1 food investigation (**B6**) directionally confirms the feed side is soft, but did not calibrate a change. |
...
| B6 | **Food production likely too generous — investigated in Phase 1, deliberately not changed.** Playtest: one Hunting Cabin + a partly-staffed Fishing Hut comfortably fed ~12 adults + 30 children (≈540 food/season). Consumption is verified sound (`FOOD_PER_CITIZEN_PER_SEASON` 20/adult, ×`CHILD_FOOD_FACTOR` 0.5 = 10/child; covered by a new sim test). Production is `LOAD_FOOD` (8) per `WORK_SECONDS` (8) cycle × circle/water factor, and by that arithmetic one food worker can feed roughly **5–15 people**, so a 3-job cabin plausibly over-supports a large settlement — matching the report. **No constant was touched:** a trustworthy per-building throughput number could not be pinned down headlessly — force-placed/cramped food buildings hit a nav/hauling stall where workers carry a load they never deposit (an artifact reproducible on *unchanged* game code, i.e. not a Phase-1 regression), so any number measured that way understates real output. The task's rule is not to change production constants without evidence. Note part of the "too easy" feeling was **runaway population**, which the Phase-1 birth cap (4/couple, declining odds) now removes — feeding pressure should already read harder than it did. | 🔵 needs-playtest | **Return-to plan:** measure real throughput in the *actual app* (Playwright: found a village, build+staff one Hunting Cabin sited near forest, advance ~4 seasons, read the Town-Hall ledger's `food` gained/season) and confirm the fed-population ratio. Then, if still too soft, apply **one** calibrated lever — lower `LOAD_FOOD` or the individual food-building yields (`GATHER/FISH/HUNT/RANCH_FOOD_PER_SEASON`, `FARM_FOOD_PER_WORKER`), **not** consumption — and re-verify early-game survival stays possible (don't make it brutal — see B3/B4). Cross-ref B3. |
